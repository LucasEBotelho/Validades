...
def cadastrar_produto():
    ...
    produto_selecionado = st.selectbox("Escolha o produto", produtos_df['Nome Produto'], key="produto_selectbox")
    
    # Se o produto não estiver na lista, adicionar um novo
    if produto_selecionado == "Adicionar Novo Produto":
        codigo_produto = st.text_input("Código do Produto", key="codigo_produto")
        nome_produto = st.text_input("Nome do Produto", key="nome_produto")
        
        # Salvar o novo produto
        if st.button("Salvar Novo Produto", key="salvar_produto"):
        ...
        data_validade = st.date_input("Data de Validade", min_value=datetime.today(), key="data_validade")
        unidade = st.selectbox("Unidade", ["Betelitas", "Lojinha", "Commulter"], key="unidade")
        quantidade = st.number_input("Quantidade", min_value=0, key="quantidade")
        valor = st.number_input("Valor", min_value=0.0, format="%.2f", key="valor")
        observacao = st.text_input("Observação", key="observacao")
        promocao = st.checkbox("Promoção?", key="promocao")
        perda = st.checkbox("Perda?", key="perda")
        
        valor_promocional = 0.0
        perda_pos_promocao = 0.0
        
        if promocao:
            valor_promocional = st.number_input("Valor Promocional", min_value=0.0, format="%.2f", key="valor_promocional")
        ...
        if st.button("Salvar Cadastro", key="salvar_cadastro"):
...
with col2:
    if st.button("Cadastrar Novo Produto", key="cadastrar_produto"):
        cadastrar_produto()